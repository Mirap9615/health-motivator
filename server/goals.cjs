...
// GET /api/goals: fetches system goals combined with user's completion status 
router.get('/', isAuthenticated, async (req, res) => {
    const userId = req.session.user.user_id;

    try {
        const systemGoalsResult = await pool.query('SELECT goal_key, goal_text, goal_type FROM system_goals ORDER BY goal_type, goal_key'); 
...
router.post('/:goalKey/toggle', isAuthenticated, async (req, res) => {
    ...
    try {
        ...
        const goalTypeResult = await client.query(
            'SELECT goal_type FROM system_goals WHERE goal_key = $1',
            [goalKey]
        );
        ...
        const periodStartDateQuery = goalType === 'daily'
            ? 'SELECT CURRENT_DATE AS period_start;'
            : 'SELECT DATE_TRUNC(\'week\', CURRENT_DATE)::DATE AS period_start;';
        ...
        const completionCheck = await client.query(
            'SELECT completion_id FROM goal_completion WHERE user_id = $1 AND goal_key = $2 AND completion_period_start = $3',
            [userId, goalKey, periodStartDate]
        );
        ...
        if (completionCheck.rows.length > 0) {
            await client.query(
                'DELETE FROM goal_completion WHERE completion_id = $1',
                [completionCheck.rows[0].completion_id]
            );
            completedStatus = false;
        } else {
            await client.query(
                'INSERT INTO goal_completion (user_id, goal_key, completion_period_start, completed_at) VALUES ($1, $2, $3, CURRENT_TIMESTAMP)',
                [userId, goalKey, periodStartDate]
            );
...
router.get('/completed/daily', isAuthenticated, async (req, res) => {
    ...
    try {
        const result = await pool.query(`
            ...
            WHERE user_id = $1
              AND goal_key IN (
                  SELECT goal_key FROM system_goals WHERE goal_type = 'daily'
              )
